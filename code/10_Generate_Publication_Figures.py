# 10_Generate_Publication_Figures.py
"""
...
# ================== Supplementary Figure S2: 消融与基线对比柱状图 ==================
def create_figure_s2_ablation_bar():
    print("\n>>> 正在生成 Figure S2 (消融与基线对比柱状图，含 3D CNN)...")
    
    models = ['End-to-End\n3D CNN', 'Global\nDecoupled', 'Traditional\nDosiomics', 'Evidential\nDosiomics']
    aucs = [0.337, 0.529, 0.675, 0.750]
    colors = ['#C0392B', '#E67E22', '#4D8CB5', '#27AE60']
    
    fig, ax = plt.subplots(figsize=(8.5, 6), facecolor='white', dpi=150)
    
    bars = ax.bar(models, aucs, color=colors, edgecolor='black', linewidth=1.2, width=0.55)
    ...
    ax.set_title('Ablation & Baseline Study: AUC Comparison', fontsize=15, fontweight='bold', pad=15)
    ax.set_ylim(0, 0.9)
    ax.grid(axis='y', linestyle=':', alpha=0.4)
    ax.legend(loc='upper left', fontsize=11, frameon=True)
    
    ax.annotate('Deep Learning\nCollapse', xy=(0, 0.337), xytext=(0.15, 0.20),
...
def create_figure_s3_roi_comparison():
    ...
    fig, ax = plt.subplots(figsize=(6.5, 6), facecolor='white', dpi=150)
    
    bars = ax.bar(rois, aucs, color=colors, edgecolor='black', linewidth=1.5, width=0.45)
    ...
    # 标注 +0.221 的巨大增益 (0.750 - 0.529)
    ax.annotate('', xy=(1, 0.750), xytext=(0, 0.529),
                arrowprops=dict(arrowstyle='<->', color='black', lw=1.5))
    ax.text(0.5, 0.64, '+0.221', ha='center', va='center', 
